...
Suunto integration is fetch-only and user-triggered. The fetch dialog offers two time ranges. "Recent days" asks for how many recent days to check and the sidecar lists workouts with `suuntool workouts list --since <days>d --limit 100 --format json`. "All time" (issue #24) instead streams the user's entire workout history with `suuntool workouts list --stream --limit 0 --quiet --timeout <ms>`, which auto-paginates every server cursor and emits NDJSON (one workout per line, no `--format`; `--quiet` keeps informational logs out of stdout). The explicit `--timeout` matters: suuntool has its own internal HTTP client timeout (30s by default per its own `--help`), entirely separate from the sidecar's Node-level kill timer below — a real "all time" fetch died at exactly that 30s default mid-stream (`BAD_ENVELOPE: unexpected end of JSON input`, suuntool exit code 5) before the sidecar's own 180s budget ever got a chance to matter, so `LIST_ALL_HTTP_TIMEOUT_MS` (10s under `SUUNTOOL_LIST_ALL_TIMEOUT_MS`) is passed explicitly so suuntool aborts itself cleanly, if it aborts at all, well before the Node-level SIGTERM would fire. Unparseable lines are counted, not thrown on — failing a multi-thousand-workout history over one bad line would discard all of its progress — so the listing only errors when nothing at all parsed; the count is logged (never the line itself) on `suunto.workouts.list_all.result`. If suuntool still exits non-zero after streaming some complete lines (a transient server/network hiccup mid-pagination, as in the incident above), those already-streamed workouts are returned as a salvaged partial listing instead of being discarded — safe because the caller already dedupes against `dives`/`suunto_imports` — but only for `server`/`timeout`/`network`-classified exits; an auth or usage failure never salvages partial output. Because that one command is the only unbounded stdout path in the sidecar, it gets both its own longer `SUUNTOOL_LIST_ALL_TIMEOUT_MS` (180s, with the client's own fetch timeout deliberately above it at 200s) and its own `SUUNTO_SIDECAR_MAX_LIST_ALL_BYTES` ceiling (50MB), which kills the child and fails closed with a 413 rather than parsing a truncated prefix. The two modes are separate, type-checked request shapes end to end — `listSuuntoWorkouts(session, { daysBack } | { all: true })` and `fetchSuuntoWorkoutsAction({ mode: "days", daysBack } | { mode: "all" })` — and the action re-validates the `mode` discriminant at runtime too, since Server Action arguments are client-controlled and an unrecognised mode must not fall through to the expensive all-time path. Everything after the listing (dedupe, export, compile, stage) is the same code for both modes.
