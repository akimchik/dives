...
test.describe("Suunto merge field preselection", () => {
  test("prefers whichever side has data, and the more precise numeric reading when both do", async ({ page }) => {
    ...
    const maxDepthRow = page.locator("div.grid", { has: page.locator('input[name="suunto-merge-maxDepth"]') });
    ...
    const maxDepthRadios = page.locator('input[name="suunto-merge-maxDepth"]');
    ...
    const buddyRadios = page.locator('input[name="suunto-merge-buddy"]');
    ...
    const notesRadios = page.locator('input[name="suunto-merge-notes"]');
